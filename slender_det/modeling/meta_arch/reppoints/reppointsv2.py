...
class RepPointsV2(nn.Module):
    ...
    def __init__(self, cfg):
        ...
        self.shift_generator = ShiftGenerator(feature_shapes)
...
class RepPointsHead(nn.Module):
    ...
    def forward(self, features):
        ...
        for l, feature in enumerate(features):
            ...
            pts_out_init = self.reppoints_pts_init_out(F.relu_(self.reppoints_pts_init_conv(reg_feat)))
            ...
            cls_outs.append(
                self.reppoints_cls_out(F.relu_(self.reppoints_cls_conv(cls_feat, dcn_offset))))
            pts_out_refine = self.reppoints_pts_refine_out(
                F.relu_(self.reppoints_pts_refine_conv(reg_feat, dcn_offset)))
