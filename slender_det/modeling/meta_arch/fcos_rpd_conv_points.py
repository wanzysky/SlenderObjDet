...
#            gt_classes, reg_targets = self.get_ground_truth(locations, gt_instances)
            losses = self.losses(init_gt_classes, init_reg_targets, refine_gt_classes, refine_reg_targets, box_cls, box_init, box_refine, ctr_sco, strides)
...
class FCOSRepPointsHead(torch.nn.Module):
    def __init__(self, cfg, input_shape: List[ShapeSpec]):
        ...
        self.points_conv = nn.Conv2d(4, points_out_dim, kernel_size=3, stride=1, padding=1)
    ...
    def forward(self, x):
        ...
        for l, feature in enumerate(x):
            ...
            if self.norm_reg_targets:
                ...
                if self.training:
                    bbox_init.append(bbox_pred)
                else:
                    bbox_init.append(bbox_pred * self.fpn_strides[l])
            else:
                #bbox_reg.append(torch.exp(bbox_pred))
                bbox_init.append(bbox_pred)
        #rep part
        offsets_init = [self.points_conv(f) for f in bbox_init]
...
#        offsets_refine = self.offsets2ltrb(offsets_refine)
        strides = [1,2,4,8,16]
        bbox_init = [torch.exp(bbox_init[f])*strides[f] for f in range(len(bbox_init))]
        bbox_refine = [torch.exp(bbox_refine[f])*strides[f] for f in range(len(bbox_refine))]
        
        return logits, bbox_init, bbox_refine, centerness
