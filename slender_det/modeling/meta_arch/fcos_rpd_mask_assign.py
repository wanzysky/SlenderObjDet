...
class FCOSRepPoints(nn.Module):
    ...
    def forward(self, batched_inputs):
        ...
        strides = [torch.full((shapes[i][0]*shapes[i][1],), self.fpn_strides[i], device=self.device) for i in range(len(shapes))]
        ...
        if self.training:
            init_gt_classes, init_reg_targets, refine_gt_classes, refine_reg_targets, gt_center_scores =\
...
#            gt_classes, reg_targets = self.get_ground_truth(locations, gt_instances)
            losses = self.losses(init_gt_classes, init_reg_targets, refine_gt_classes, \
                refine_reg_targets, box_cls, box_init, box_refine, ctr_sco, strides, gt_center_scores)
    ...
    def losses(self, init_gt_classes, init_reg_targets, refine_gt_classes, refine_reg_targets,\
        pred_class_logits, pred_box_reg_init, pred_box_reg, pred_center_score, strides, gt_center_score):
...
#        ) / sum_centerness_targets_avg_per_gpu 
    ...
    def get_ground_truth(self, points: torch.Tensor, init_boxes, gt_instances):
        ...
        valid_ids = (init_gt_classes >= 0) & (init_gt_classes != self.num_classes)
        gt_center_scores = []
        centers = torch.cat(points,0)#[X,2]
        for im_i in range(len(gt_instances)):
            valid_id = valid_ids[im_i].nonzero().view(-1)
            valid_locations = centers[valid_id].long()
            center_mask= gt_center_masks[im_i]
            center_mask = torch.Tensor(center_mask).to(self.device)
            gt_center_score = center_mask[valid_locations[:,1],valid_locations[:,0]]#valid_location:xy, centers:[h,w]
        ...
        return init_gt_classes, init_reg_targets, refine_gt_classes, refine_reg_targets, gt_center_scores
