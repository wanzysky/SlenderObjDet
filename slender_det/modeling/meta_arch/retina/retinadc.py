...
class DeformableConvRetinaNetHead(nn.Module):
    ...
    def forward(self, features):
        ...
        for i in range(len(features)):
            ...
            box_reg_i_xyxy, bbox_out_init = self.gen_grid_from_reg(reg,
                                                                   bbox_init.detach())  # box_reg_i_xyxy: [b,18,h,w]
            ...
            for conv_i in range(self.num_anchors):
                deform_cls_conv.append(self.deform_cls_conv(cls_features[i],
                                                            dcn_offset[conv_i * batch_size:(conv_i + 1) * batch_size, :,
                                                            :, :]))
                deform_reg_conv.append(self.deform_reg_conv(reg_features[i],
                                                            dcn_offset[conv_i * batch_size:(conv_i + 1) * batch_size, :,
                                                            :, :]))
            ...
            offsets_final_delta = torch.cat(
                [offsets_final_delta_x, offsets_final_delta_y, offsets_final_delta_w, offsets_final_delta_h], dim=1)
