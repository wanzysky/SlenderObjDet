...
class FCOSRepPoints(nn.Module):
    ...
    def forward(self, batched_inputs):
        ...
        if self.training:
            ...
            losses = self.losses(gt_classes, reg_targets, box_cls, box_init, box_refine, ctr_sco)
    ...
    def losses(self, gt_classes, reg_targets, pred_class_logits, pred_box_reg_init, pred_box_reg, pred_center_score):
        ...
        if pos_inds.numel() > 0:
            ...
            reg_loss_init = iou_loss(
                pred_box_reg_init[foreground_idxs], reg_targets[foreground_idxs], gt_center_score,
            ...
            reg_loss_init = pred_box_reg_init[foreground_idxs].sum()
...
class FCOSRepPointsHead(torch.nn.Module):
    ...
    def offsets2ltrb(self, deltas: List[torch.Tensor], point_strides=[1, 1, 1, 1, 1]):
        """
        Args:
            deltas: List[[H*W,2]] coordinate of each feature map
            point_strides: List[[N,C,H,W]] offsets
