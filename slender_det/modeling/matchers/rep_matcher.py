...
def rep_points_match_with_classes(centers, strides, boxes, gt_labels, num_classes=80, scale=4, pos_num=1):
    """Assign gt to points.
    ...
    assigned_labels = assigned_gt_inds.new_zeros((num_points,)) + num_classes
    pos_inds = torch.nonzero(assigned_gt_inds > 0).squeeze()
    if pos_inds.numel() > 0:
